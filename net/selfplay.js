...
for (let g = 0; g < nGames; g++) {
  ...
  let buf = "";
  for (const r of recs) { const z = w === "draw" ? 0 : (((w === "black" ? 1 : 2) === r.turn) ? 1 : -1); buf += JSON.stringify({ c: r.c, turn: r.turn, ko: r.ko, pi: r.pi, z }) + "\n"; }
  fs.appendFileSync(outFile, buf);
  totalPos += recs.length;
  process.stderr.write(`[${outFile}] game ${g + 1}/${nGames} moves ${mv} winner ${w} +${recs.length} (tot ${totalPos})\n`);
}
